...
async function run(command, { silent = false, allowFailure = false } = {}) {
  if (!silent) {
    log(`${colors.yellow}$ ${command}${colors.reset}`);
  }
...
// Helper to check if a URL responds with expected status
...
import { 
  ...
  cleanupDockswapContainers,
  cleanupDockswapDatabase
} from './utils.js';
...
async function testBasicDeployment() {
  logStep(2, "Testing basic deployment flow");
...
async function testStatus() {
  logStep(3, "Testing status command");
...
async function testBlueGreenDeployment(currentColor) {
  logStep(4, "Testing blue-green deployment");
...
async function testTrafficSwitching(targetColor) {
  logStep(5, "Testing traffic switching");
...
async function cleanup() {
  logStep(6, "Cleaning up test environment");
